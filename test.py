...
def compare_lr_and_rt():
    ...
    data_file_path = os.path.join(os.path.abspath(os.curdir), 'data/0003.HK.csv')
    window_size = 5
    ...
    open_df = pd.DataFrame(open_price, columns=column)
    close_df = pd.DataFrame(close_price, columns=column)
    plot_label_vs_data(data=close_price, label=column, graph_index=0,
                       graph_title="close price compare", plt=plt)
    plot_label_vs_data(data=open_price, label=column, graph_index=1,
                       graph_title="0003.HK Price Prediction", plt=plt)
    plt.show()
